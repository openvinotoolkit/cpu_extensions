...
#include "rotary_kernel_avx2.hpp"
...
using RotaryTestAVX2ParamSet = std::tuple<
        data_type_t                                // data type
        >;

class RotaryTestAVX2 : public TestWithParam<RotaryTestAVX2ParamSet> {
public:
    static std::string getTestCaseName(const testing::TestParamInfo<RotaryTestAVX2ParamSet>& obj) {
        data_type_t types;
...
protected:
    ...
    void test(float thresh) {
        for (int n = 6; n < 129; n += 2) {
            ...
            rotary_avx2(n, cos.data, sin.data, q_src.data, k_src.data, q_dst.data, k_dst.data);
            for (int i = 0; i < n; i++) {
                float q = q_dst[i];
...
INSTANTIATE_TEST_SUITE_P(smoke_Rotary, RotaryTestAVX2,
    ...
    RotaryTestAVX2::getTestCaseName);
